var Input = {
  ...
  class Segment {
    ...
    follow(iter) {
      ...
      if (dist < 0.001) return; // FIX: guard against zero-distance NaN
  ...
  function setupLizard(size, legs, tail) {
    ...
    for (var i = 0; i < tail; i++) {
      ...
      for (var ii = -1; ii <= 1; ii += 2) {
        var node = new Segment(spinal, s * 3, ii, 0.1, 2);
        for (var iii = 0; iii < 3; iii++) {
          // FIX: clamp minimum size to 1 to prevent zero-size segments
          node = new Segment(node, Math.max(1, s * 3 * (tail - i) / tail), -ii * 0.1, 0.1, 2);
        }
      }
    }
    setInterval(function() {
